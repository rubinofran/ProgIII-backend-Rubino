...
/*     const requestQuery = () => (req, res, next) => {
  ...
  _setupExpressRoutes() {
    ...
    this.app.use('/transactions', routers.transaction)

    this.logger.verbose('Enrutadores de recursos adjuntados')
  }

  // req.logger not found ...
  _setupErrorHandler() {
    ...
    this.app.use((err, req, res, next) => {
      ...
      if (!err.statusCode) {
        err.statusCode = Server.statusCodeByErrorName[err.name] || Server.statusCodeByErrorName[DefaultError]
      }
      ...
      if (isMongooseValidationError) {
        ...
        // console.log('DEBUG --------->', 'Error de validación de mongoose', JSON.stringify(err)) // **
        ...
        // console.log('VERBOSE --------->', 'Respondiendo al cliente: ', err.toString()) // **
